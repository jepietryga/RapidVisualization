...
void SaveFunction(object given, number fieldNo, string provided)
    {
        ...
        while(pos < len(save_string))
        {
            ...
            //D,Date; T,Time; V, Voltage; M, mag; L, length; o, Mode; R, Brightness; S spot; A alpha; B beta; P Provided
...
TagGroup MakeSaveDialog()
    {
        ...
        TagGroup SaveStringField = DLGCreateStringField("%P%_%A%degrees_%X%um_%Y%um_%Z%um_%M%X_%L%mm",80).dlgidentifier("SaveStringField");
...
void captureFunction_Reference(object given, string Cycler)
{
	...
	ShowImage(stack)
	cycle(Cycler)
	result("Reference Capture Complete!\n")
}
...
void RefreshFunction_next(object given, number setting, number xA,number xB, number xC, number xD,number yA, number yB, number yC, number yD,number zA, number zB, number zC, number zD,number dfA, number dfB, number dfC, number dfD){
	...
	dlgValue(given.lookupelement("nextTilt"), EMGetStageAlpha())
	//string debugVals = " "
